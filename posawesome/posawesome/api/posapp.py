...
def get_items(pos_profile, price_list=None, item_group="", search_value=""):
    ...
    def _get_items(pos_profile, price_list, item_group, search_value):
        ...
        items_data = frappe.get_all("Item", filters=filters, fields=fields)

        #get all notes in item from childtable custom_notes from doctype Notes
        for item in items_data:
            item["notes"] = get_item_notes(item.item_code)
            bundles = get_product_bundle(item.item_code)
            if bundles != None:
                item["custom_bundle_options"] = bundles
